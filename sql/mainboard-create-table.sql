...
(
    ...
    DEFAULT_ADDRESS_YN    CHAR(1) NOT NULL,
    COMMON_ENTRANCE_ACCESS_NUMBER_YN    CHAR(1) NOT NULL,
...
COMMENT ON COLUMN TB_CUPON.COUPON_CODE IS '쿠폰코드';

COMMENT ON COLUMN TB_CUPON.COUPON_NAME IS '쿠폰이름';

COMMENT ON COLUMN TB_CUPON.EXPLANATION IS '쿠폰설명';

COMMENT ON COLUMN TB_CUPON.EXPIRATION_PERIOD IS '유효기간';

COMMENT ON TABLE TB_CUPON IS '쿠폰';

CREATE UNIQUE INDEX TB_CUPON_PK ON TB_CUPON
( COUPON_CODE );

ALTER TABLE TB_CUPON
 ADD CONSTRAINT TB_CUPON_PK PRIMARY KEY ( COUPON_CODE )
 USING INDEX TB_CUPON_PK;
...
COMMENT ON COLUMN TB_MEMBER.ADDRESS_CODE IS '배송지코드';
